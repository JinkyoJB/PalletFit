...
class FFD_M(Base):
    ...
    def addItem(self, bin, item, test=False):
        ...
        for axis in range(0,3):
            ...
            for ib in item_in_bin:
                for rt in rotate:
                    ...
                    available_pivot.append([pivot, rt])

        # available_pivot을 [2]가 작은 순, [0]이 작은 순, [1]이 작은 순으로 정렬
        available_pivot = sorted(
            available_pivot,
            key=lambda p: (
                p[0][0]**2 + p[0][1]**2,  # (1) XY 평면상의 제곱 거리 오름차순
                p[0][1],               # (2) y 좌표 오름차순
                p[0][0],               # (3) x 좌표 오름차순
                p[0][2]                # (4) z 좌표 오름차순
            )
        ...
        for pivot in available_pivot:
            fitted, loaded_item = checkPivot_R(bin, item, pivot[0], pivot[1])
            if fitted > 0:
                if test:
                    return fitted, None
                bin.store(loaded_item)
