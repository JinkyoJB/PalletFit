...
from planning.RL.DQN.src import Tetris3D
from planning.RL.DQN.src import Net3DSA as Net3D
from planning.RL.DQN.src import VGG3D, VGG3DSA
from planning.RL.DQN.utils.utils import Log, plot_3d_volume

# planning 내부용 import
# from utils.constants import RotationType
# from RL.DQN.src import Tetris3D
# from RL.DQN.src import Net3DSA as Net3D
# from RL.DQN.src import VGG3D, VGG3DSA
# from RL.DQN.utils.utils import Log, plot_3d_volume

# from torch.optim.lr_scheduler import CosineAnnealingLR
# import matplotlib # planning내부용. GUI모드에서는 주석필요
# matplotlib.use("TkAgg")   # planning내부용. GUI모드에서는 주석필요
...
class Agent:
    ...
    def set_network(self, weight_path):
        ...
        if weight_path is not None:
            ...
            main_model = VGG3DSA(input_shape=(self.depth, self.height, self.width), 
                                 output_size=output_size, 
                                 item_feat_dim=item_feat_dim).to(self.device)
        ...
        self.target_q_network = VGG3DSA(input_shape=(self.depth, self.height, self.width),
                                 output_size=output_size, 
                                 item_feat_dim=item_feat_dim).to(self.device)
        self.target_q_network.load_state_dict(self.main_q_network.state_dict())
    ...
    def get_minibatch(self):
        ...
        # 데이터 분리
        state_batch, action_batch, reward_batch, next_state_batch, done_batch, item_batch, next_item_batch = zip(*batch)

        # 텐서 변환
        state_batch = torch.stack(state_batch).unsqueeze(1).to(self.device)  # [batch_size, 1, depth, height, width]
        ...
        next_state_batch = torch.stack(next_state_batch).to(self.device)  # [batch_size, 1, depth, height, width]
        done_batch = torch.tensor(done_batch, dtype=torch.bool).unsqueeze(1).to(self.device)  # [batch_size, 1]
        item_batch = torch.stack(item_batch, dim=0).to(self.device)  # [batch_size, 3]
        next_item_batch = torch.stack(next_item_batch, dim=0).to(self.device)  # [batch_size, 3]

        return state_batch, action_batch, reward_batch, next_state_batch, done_batch, item_batch, next_item_batch



class DQN:
    def __init__(self,  unfit_stop_setting = True, rotation_quat = RotationType.BasicRotation, use_cuda = True):
        self.unfit_stop_setting = unfit_stop_setting
        self.rotation_quat = rotation_quat
        self.weight_path = "planning/RL/DQN/weight/weightFiles3D_20250212-065936_202020_20000_item_info_200/model_best.pth"
        ...
        self.agent = Agent(
            ...
            load_weight=self.weight_path,
                ...
                # Tetris3D 환경 생성
        self.env = Tetris3D(width=self.width, height=self.height, depth=self.depth, render=False, mode = 'train')
    ...
    def train(self, target_bins, items_list, weight_path, nickname = 'item_info', lr = 0.0005):
        ...
        state = env.reset()# env.reset().unsqueeze(0).unsqueeze(1).to(self.device)  # [1,1,20,20,20]
        ...
        while step < self.steps:
            ...
            if len(next_states) == 0:
                ...
                state = env.reset()  # [20,20,20]
            else:
                next_actions, values = zip(*next_states.items())    # ex: next_actions: ((0, 0), (1, 0), (2, 0), (3, 0), (4, 0), (0, 1), (1, 1), (2, 1), (3, 1), (4, 1))
                next_states_bins, next_states_item = zip(*values)   # ex: next_states_bins.shape: (20,20,20), next_states_item: (3,)
                next_states_bins = torch.stack(next_states_bins).unsqueeze(1).to(self.device)   # [num_available, 20, 20, 20]
                next_states_item = torch.stack(next_states_item).to(self.device)   # [num_available, 3]

                self.agent.main_q_network.eval()
                with torch.no_grad():
                    # q_values = self.agent.main_q_network(next_states)  # [num_available, 2]
                    q_values = self.agent.main_q_network(next_states_bins, next_states_item)[:,0]
                self.agent.main_q_network.train()

                if is_random:
                    selected_idx = randint(0, len(next_states_bins) - 1)
                else:
                ...
                next_state = next_states_bins[selected_idx]  # [1,1,20,20,20]
                next_item_info = next_states_item[selected_idx]  # [1,3]
                # print(f"Selected next state shape: {next_state.shape}")  # 디버깅용 출력
                item_info = env.get_item_info()
                reward, truncated, terminaled = env.step(action)
                done = truncated or terminaled
                state = torch.tensor(env.bin_matrix, dtype=torch.float32, device=self.device)
                ...
                if selected_idx >= 0 and env.num_pieces > 2:
                    ...
                    self.agent.replay_memory.append([state, selected_idx, reward, next_state, done, item_info, next_item_info])
            ...
            if len(self.agent.replay_memory) >= self.batch_size:
                state_batch, action_batch, reward_batch, next_state_batch, done_batch, item_batch, next_item_batch = self.agent.get_minibatch()

                # Q(s,a) 계산
                q_values = self.agent.main_q_network(state_batch, item_batch)  # [batch_size, 6]
                ...
                self.agent.main_q_network.eval()
                with torch.no_grad():
                    next_prediction_batch = self.agent.target_q_network(next_state_batch, next_item_batch)
                ...
                if step % 2000 == 0:
                    ...
                    available = self.stack(bin_for_training, items_list)
    ...
    def stack(self,current_bin, items_list):
        '''
        items_list를 target_bin에 적재하고 적재 여부를 반환합니다.
        input: current_bin, items_list
        output: fit_result
        '''
        ...
        # 기존에 있던 target_bin의 아이템 store
        for item in current_bin.get_all_items():
            w,h,d = item.getDimension()
            w = np.ceil(w / x_scale)
            h = np.ceil(h / y_scale)
            d = np.ceil(d / z_scale)
            item_shape = [w,h,d]
            item_id = self.env.set_new_piece(item_shape)      
            piece = self.env.tetromino3d.pieces_detail[item_id]

            item_x, item_y, item_z = item.b_position

            x = np.ceil(item_x / x_scale)
            y = np.ceil(item_y / y_scale)
            z = np.ceil(item_z / z_scale)
            pos = {'x':x, 'y':y, 'z':z}
            self.env.bin_matrix = self.env.store(piece, pos)  

        # self.env.plot_piece(plot_name='env_set')        
        ...
        for idx, item in enumerate(items_list):
            ...
            if not next_states:
                ...
                with torch.no_grad():
                    q_values = self.agent.main_q_network(next_states_bins, next_states_item)[:, 0]    # [num_next_actions, 2]
        ...
        # self.env.plot_piece(plot_name='env_stacked')
        return fit_result
